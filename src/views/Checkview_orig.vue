<template>
  <div class="profile mt-4 sm:mt-14 mb-8 mx-auto border max-w-4xl rounded-xl shadow-lg overflow-hidden p-8">
   ...
   <!-- <p>{{message}}</p> -->
        ...
        <div class="grid grid-cols-4 gap-4 mb-4 sm:mb-4">
          <label for="profile-first-name" class="text-medium font-medium text-message my-auto"> Check Issuance Date:</label>
          <div class="col-span-2">
           {{che_date}}       
          </div>  
        </div>
        <div class="grid grid-cols-4 gap-4 mb-4 sm:mb-4">
          <label for="profile-first-name" class="text-medium font-medium text-message my-auto"> Amount and Check Issuance Dates:</label>
          <div class="col-span-2">
           <!--<ul id="example-1">
              <li v-for="message in messages" :key="message.amount">
              {{ message.amount }}
              </li>
            </ul>
            {{messages}}-->
            <ul>
              <li v-for="index in count" :key="index">
                Amount: {{ amount[index-1] }} -- Check date: {{ che_date[index-1] }}
              </li>
            </ul>
            <!--<ul>
              <li v-for="index in count" :key="index">
                Amount: {{ check[index-1].amount }} -- Check date: {{ che_date[index-1] }}
              </li>
            </ul>-->
          </div>  
        </div>
